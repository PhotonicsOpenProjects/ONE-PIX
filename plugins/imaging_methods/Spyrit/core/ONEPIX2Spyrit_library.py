...
class onepix2spyrit :
    ...
    def cnn32to64_hadam_spyrit_reconstruct(self,device='cpu'):
        ...
        root.withdraw()
        root.attributes('-topmost', 1)
        cnn_path = filedialog.askdirectory(title = "Select the folder containing cnn model", initialdir = actual_path)
        os.chdir(cnn_path)

        Cov_rec = np.load(cnn_path+'\Cov_64x64.npy')
        title=cnn_path+'\cnn_32to64'

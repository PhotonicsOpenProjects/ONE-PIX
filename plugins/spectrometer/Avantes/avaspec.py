...
DSTR_STATUS_DSS_MASK = (
    0x01  # DSTR Sequence Stop (DSS) bit of MEASUREMENT_DSTR_STATUS->DMS
)
DSTR_STATUS_FOE_MASK = (
    0x02  # FIFO Overflow Error (FOE) bit of MEASUREMENT_DSTR_STATUS->DMS
)
DSTR_STATUS_IERR_MASK = (
    0x04  # Internal Error (IERR) bit of MEASUREMENT_DSTR_STATUS->DMS
)
...
if "linux" in sys.platform:  # Linux will have 'linux' or 'linux2'
    ...
    lib = ctypes.CDLL(root_path + os.sep + "libavs.so")
    func = ctypes.CFUNCTYPE
elif "darwin" in sys.platform:  # macOS will have 'darwin'
    lib = ctypes.CDLL(root_path + os.sep + "libavs.0.dylib")
    func = ctypes.CFUNCTYPE
    ...
    if ctypes.sizeof(ctypes.c_voidp) == 8:  # 64 bit
        ...
        lib = ctypes.WinDLL(root_path + os.sep + "avaspecx64.dll")
        func = ctypes.WINFUNCTYPE
    else:
        WM_MEAS_READY = 0x0401
        lib = ctypes.WinDLL(root_path + os.sep + "avaspec.dll")
        func = ctypes.WINFUNCTYPE
...
def AVS_GetNrOfDevices():
    ...
    """
    prototype = func(ctypes.c_int)
    AVS_GetNrOfDevices = prototype(
        ("AVS_GetNrOfDevices", lib),
    )
...
def AVS_UpdateUSBDevices():
    ...
    """
    prototype = func(ctypes.c_int)
    AVS_UpdateUSBDevices = prototype(
        ("AVS_UpdateUSBDevices", lib),
    )
...
def AVS_UpdateETHDevices(spectrometers=1):
    ...
    prototype = func(
        ...
        ctypes.POINTER(BroadcastAnswerType * spectrometers),
    )
    paramflags = (
        (
            1,
...
def AVS_GetList(spectrometers=1):
    ...
    prototype = func(
        ...
        ctypes.c_int,
        ctypes.POINTER(ctypes.c_int),
        ctypes.POINTER(AvsIdentityType * spectrometers),
    )
    paramflags = (
        (
            1,
...
def AVS_Activate(deviceId):
    ...
    while x < 74:  # 10 through 73
        temp[x] = 0
        x += 1
    temp[74] = int.from_bytes(
        deviceId.Status, byteorder="big"
...
def AVS_Measure(handle, windowhandle, nummeas):
    ...
    if not (("linux" in sys.platform) or ("darwin" in sys.platform)):
        prototype = func(
            ctypes.c_int, ctypes.c_int, ctypes.wintypes.HWND, ctypes.c_uint16
        )
    else:
...
class AVS_MeasureCallbackFunc(object):
    def __init__(self, function):
        self.prototype = ctypes.CFUNCTYPE(
...
class AVS_DstrCallbackFunc(object):
    def __init__(self, function):
        self.prototype = ctypes.CFUNCTYPE(
            None, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_uint)
        )
        self.callback = self.prototype(function)
...
def AVS_GetDigIn(handle, portId):
    ...
    """
    prototype = func(
        ctypes.c_int, ctypes.c_int, ctypes.c_uint8, ctypes.POINTER(ctypes.c_uint8)
    )
    paramflags = (
        (
            1,
...
def AVS_SetPwmOut(handle, portId, frequency, dutycycle):
    ...
    """
    prototype = func(
        ctypes.c_int, ctypes.c_int, ctypes.c_uint8, ctypes.c_uint32, ctypes.c_uint8
    )
    paramflags = (
        (
            1,
...
def AVS_GetAnalogIn(handle, portId):
    ...
    """
    prototype = func(
        ctypes.c_int, ctypes.c_int, ctypes.c_uint8, ctypes.POINTER(ctypes.c_float)
    )
    paramflags = (
        (
            1,
...
def AVS_GetDetectorName(handle, SensorType):
    ...
    """
    prototype = func(
        ctypes.c_int, ctypes.c_int, ctypes.c_byte, ctypes.c_char * DETECTOR_NAME_LEN
    )
    paramflags = (
        (
            1,

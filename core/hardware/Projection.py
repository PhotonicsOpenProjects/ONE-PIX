...
class Projection:
    ...
    def create_integrated_frame(self):
        ...
        cv2.setWindowProperty("ImageWindow", cv2.WND_PROP_FULLSCREEN, 1)
        cv2.moveWindow(
            "ImageWindow", screenWidth + self.proj_position[0], self.proj_position[1]
        )
    ...
    def get_integration_time_auto(self, acq_config):
        ...
        acq_config.periode_pattern = int(
            acq_config.hardware.repetition
            * acq_config.hardware.spectrometer.integration_time_ms
        )
    ...
    def init_projection(self, patterns, patterns_order, interp_method):
        ...
        (
            self.create_fullscreen_window()
            if np.all(self.proj_position == "auto")
            else self.create_integrated_frame()
        )
        cv2.imshow(
            "ImageWindow",
            cv2.resize(
                patterns[0], (self.width, self.height), interpolation=interp_method
            ),
        )
        cv2.waitKey(
            3000
    ...
    def thread_projection(self, event, patterns, patterns_order, interp_method):
        ...
        for count, pattern in enumerate(patterns):
            ...
            cv2.imshow(
                "ImageWindow",
                cv2.resize(
                    pattern, (self.width, self.height), interpolation=interp_method
                ),
            )

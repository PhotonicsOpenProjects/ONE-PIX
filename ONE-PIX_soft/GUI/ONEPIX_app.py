...
button_wind_test = tk.Button(w1, text="Windows size test", command=partial(window_size_test, config),
                             bg="cornflowerblue", height=1, width=20)
button_wind_test.place(x=pos_paramX + 20, y=pos_pretestY + 75)
# button_wind_test["state"] = "disabled"

button_co = tk.Button(w1, text="Spectrometer connection", command=partial(connection, config), bg='chartreuse',
                      height=1, width=24)
button_co.place(x=pos_coX, y=pos_coY + 90)

button_acquire_hyp = tk.Button(w1, text="Acquire hypercube", command=partial(acquire_hyp, config), bg='gold', height=2,
                               width=24)
button_acquire_hyp.place(x=pos_coX, y=pos_acquirY)
button_acquire_hyp["state"] = "disabled"

button_deco = tk.Button(w1, text="Spectrometer disconnection", command=partial(disconnection, config), bg='tomato',
                        height=1, width=24)
button_deco.place(x=pos_coX, y=pos_coY + 125)

button_acquire_spec = tk.Button(w1, text="Acquire spectrum", command=partial(draw_spectrum, config),
                                bg='cornflowerblue', height=1, width=20)
...
def rogn():
    global data_cube
    global wavelengths
    data_cube, wavelengths = hsa.clip_datacube(data_cube, wavelengths, round(float(entry_borne_min.get())),
                                               round(float(entry_borne_max.get())))
    rgb_display(data_cube, wavelengths)

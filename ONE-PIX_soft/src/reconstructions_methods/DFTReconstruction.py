...
class DFTReconstruction:
    ...
    def datacube_reconstruction(self):
        ...
        left_spectrum=np.rot90(np.conjugate(half_spectrum[:,1:,:]),2) #creation of the half spatial left spectrum by conjugate symmetry
        ...
        hyperspectral_image=np.fft.fftshift(np.abs(np.fft.ifftn(whole_spectrum,axes=(0,1)))) #0 calculation of the hyperspectral image

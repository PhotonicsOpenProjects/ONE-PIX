...
class HadamardPatterns:
    ...
    def creation_patterns(self):
        ...
        hadamard_matrix=hadamard(self.dim**2,dtype=np.int16) # initialized an dim by dim Hadamard matrix
        patterns=np.zeros([self.dim,self.dim,self.dim**2],dtype=np.int16)   # 3D Patterns initialization with zeros values

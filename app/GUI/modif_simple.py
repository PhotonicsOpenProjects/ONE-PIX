...
class OPApp(ctk.CTk):
    def __init__(self):
        ...
        self.appearence_frame.grid(
            row=1, column=0, rowspan=1, padx=(10, 10), pady=(20, 0), sticky="we"
        )
    ...
    def window_size_test(self):
        ...
        self.proj.geometry(
            "{}x{}+{}+{}".format(
                self.acq_config.width, self.acq_config.height, screenWidth, 0
            )
        )
        self.proj.update()
        y = list(
            range(self.acq_config.height)
        )  # horizontal vector for the pattern creation
        x = list(
            range(self.acq_config.width)
    ...
    def spec_connection(self):
        ...
        if test == 0:
            if messagebox.askokcancel(
                self.widgets_text["specific_GUI"]["complete"]["Acquisition_tab"][
                    "functions"
                ]["askokcancel_stubspectro"]["title"],
                self.widgets_text["specific_GUI"]["complete"]["Acquisition_tab"][
                    "functions"
                ]["askokcancel_stubspectro"]["confirm"],
            ):
    ...
    def acquire_hyp(self):
        ...
        est_end = (
            ...
            + datetime.timedelta(minutes=round(self.acq_config.est_duration))
        ).strftime("%H:%M:%S")
        ...
        #Save reconstructed data with RGB image from RGB camera
        self.acq_res.save_reconstructed_image(
            f"ONE-PIX_app_{fdate}_{actual_time}", "../Hypercubes"
        )
        self.acq_config.hardware.camera.get_image(tag=None, save_path=camera_save_path)

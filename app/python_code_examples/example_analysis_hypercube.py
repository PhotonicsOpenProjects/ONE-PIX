...
rgb_image = fis.RGB_reconstruction(
...
image_seg = fis.clustering(
    datacube, 3, 3
)  #  segmentation process using kmeans  with 3 clusters on the 3 first dimensions of PCA
...
image_reshape = np.reshape(
    datacube, (np.size(datacube, 0) * np.size(datacube, 1), np.size(datacube, 2))
)
pca = PCA(3)
...
ax.scatter(
    principalComponents[np.where(kmeans.labels_ == 0), 0],
    principalComponents[np.where(kmeans.labels_ == 0), 1],
    principalComponents[np.where(kmeans.labels_ == 0), 2],
    "r",
)
ax.scatter(
    principalComponents[np.where(kmeans.labels_ == 1), 0],
    principalComponents[np.where(kmeans.labels_ == 1), 1],
    principalComponents[np.where(kmeans.labels_ == 1), 2],
    "r",
)
ax.scatter(
    principalComponents[np.where(kmeans.labels_ == 2), 0],
    principalComponents[np.where(kmeans.labels_ == 2), 1],
    principalComponents[np.where(kmeans.labels_ == 2), 2],
    "r",
)

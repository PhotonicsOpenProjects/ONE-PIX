...
def check_args(args):
    ...
    for arg in vars(args):
        ...
        if value is None:
            ...
            print('-'*100)
            print(f"{arg} not specified by user. Default value {acquisition_json_object[arg]} taken from conf/acquisition_parameters.json")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="ONE-PIX acquisition command line script",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-m', '--imaging_method',type=str,
                        help='imaging_method')
    parser.add_argument('-r', '--spatial_res',type=int,
                        help='Spatial resolution')
    parser.add_argument('-t', '--integration_time_ms',type=int,
                        help='Spectrometer integration time (ms)')
    parser.add_argument('-l', '--wl_lim',type=list,
                        help='Wavelengths range')
    parser.add_argument('-H', '--height',type=int,
                        help='Projection window height')
    parser.add_argument('-W', '--width', type=int,
                        help='Projection window height')
    parser.add_argument('-s', '--spectro_scans2avg', type=int,
